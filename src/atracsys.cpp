...
Atracsys::Atracsys(const char* directory_geometries){
    ...
    BOOST_FOREACH( boost::filesystem::path const &p, std::make_pair(it,eod)){ 
        if( is_regular_file(p)){
        	...
        	cout << "File geometria n." << count_ <<" :" <<  file_geometry << endl;
...
bool Atracsys::startTracking(){
		...
		loop_thr->detach();
		std::cout << "Thread per il tracking lanciato" << endl;
		return true;
}
...
do{
	...
	cout.setf( std::ios::fixed, std::ios::floatfield );
		...
		/* block up to 100 ms if next frame is not available*/
			...
			frame_n_++;
			cout << "No marker on frame: " << frame_n_ << endl;
...
bool Atracsys::getGeometries(std::vector<Eigen::Matrix4f> &out){
